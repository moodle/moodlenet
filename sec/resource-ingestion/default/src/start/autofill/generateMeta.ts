...
import type { LearningOutcome, ProvidedGeneratedData, ResourceDoc } from '@moodlenet/core-domain/resource'
import { callOpenAI } from '../../resource-categorize/open-ai-call'
    ...
    meta: {
      ...
      originalPublicationInfo:
        openAiResponse.data.creationYear && openAiResponse.data.creationMonth
          ...
          : null,
      language: openAiResponse.data.languageCode ? { code: openAiResponse.data.languageCode } : null,
      level: openAiResponse.data.iscedGradeCode ? { code: openAiResponse.data.iscedGradeCode } : null,
      subject: openAiResponse.data.iscedFieldCode ? { code: openAiResponse.data.iscedFieldCode } : null,
      type: openAiResponse.data.resourceTypeCode ? { code: openAiResponse.data.resourceTypeCode } : null,
    },
  }

  return {
    generatedData,
    provideImage: doc.image ? undefined : openAiResponse.resourceIngeston.provideImage,
  }
}
