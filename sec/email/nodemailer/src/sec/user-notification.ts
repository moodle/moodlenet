...
type deps = {
  filestoreHttp: DeploymentInfo
  orgInfo: OrgInfo
}
export function user_notification_service_factory(env: NodemailerSecEnv): secondaryBootstrap {
  return ({ log }) => {
    return secondaryCtx => {
      ...
      async function getEmailLayoutProps({
        ...
        if (data.module === 'iam') {
          ...
          const [found, user] = await secondaryCtx.mod.iam.query.userBy({ userId: data.toUserId, by: 'id' })
          if (!found) {
            log('warn', `User not found for id ${data.toUserId}`)
          ...
          if (data.type === 'resetPasswordRequest') {
            return [
              true,
              { receiverEmail, props: resetPasswordEmail({ resetPasswordUrl: data.resetPasswordUrl, siteName }) },
            ]
          }
        }
