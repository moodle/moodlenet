...
export function provideQueueService<jobData>({
  ...
  stopSchedulers()
  let batchSemaphore: 'green' | 'red' = 'green'
  ...
  async function consumeBatch() {
    ...
    if (batchSemaphore === 'red' || amount < 1) {
      return
    }

    batchSemaphore = 'red'
    const jobDocs = await fetchAndEngageSomeEnqueuedJobs({
      jobCollection,
      jobName,
      amount,
      parallelism,
