...
const defaultLinkIngestor: LinkIngestor = async ({ linkUrl }) => {
...
async function openGraphScrape(
  url: string,
): Promise<null | Pick<resourceIngestionMetadata, 'provideImage' | 'content' | 'title'>> {
  try {
...
export default defaultLinkIngestor
...
async function puppeteerScrape(
  url: string,
): Promise<null | Pick<resourceIngestionMetadata, 'provideImage' | 'title' | 'content'>> {
  try {
