...
import { FN_NAME, bcAttr, par } from './types'
    ...
    data.bloomsCognitive = data.bloomsCognitive
      ?.map(generatedBloom => {
        const bloomDef = prompts.bloomsCognitivesFineTuning.data.find(
          ([, verbs, levelCode]) =>
            levelCode === generatedBloom.bloomsLevelCode && verbs.includes(generatedBloom.learningOutcomeVerbCode),
        )
        ...
        if (bloomDef) {
          ...
          sanitizedGeneratedBloom.learningOutcomeDescription = generatedBloom.learningOutcomeDescription
            .trim()
            .replace(verbNail, '')
            .trim()
        }
...
and the most suitable natural language for descriptive parameters ("${par(
        'resourceTitle',
      )}", "${par('resourceSummary')}", "${par('bloomsCognitive')}.${bcAttr('learningOutcomeDescription')}") `,
      parameters: {
        type: 'object',
        properties: {
          [par('resourceTitle')]: {
            ...
            items: {
              ...
              properties: {
                ...
                [bcAttr('learningOutcomeVerbCode')]: {
                  ...
                  enum: [...new Set(prompts.bloomsCognitivesFineTuning.data.map(([, verbCode]) => verbCode).flat())],
                },
              ...
              required: [bcAttr('bloomsLevelCode'), bcAttr('learningOutcomeVerbCode'), bcAttr('learningOutcomeDescription')],
    ...
    const completionConfig: Pick<ChatCompletionCreateParams, 'messages' | 'functions' | 'function_call'> = {
      messages,
