...
export type NewUserConfirmAdapter = {
  ...
  createNewProfile(profileid: Id): Promise<unknown>
}
export const newUserConfirm = QMMutation(
  ({ token, password: plainPwd, username }: { token: string; password: string; username: string }) => async ({
    activateUser,
    hashPassword,
    generateProfileId,
    createNewProfile,
