...
export const bindString = (_: { edgeType: EdgeType; from: string; to: string }) =>
  `${nodeTypeFromId(_.from)} ${_.edgeType} ${nodeTypeFromId(_.to)}`

export const cantBindMessage = ({ edgeType, from, to }: { edgeType: EdgeType; from: string; to: string }) =>
