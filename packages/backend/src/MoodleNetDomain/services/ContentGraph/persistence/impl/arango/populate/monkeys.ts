...
getDB().then(async db => {
  ...
  function monkeyCreateNode(ctx: MoodleNetExecutionContext) {
    ...
    const info = `[${nodeType}]`
    return api<MoodleNetDomain>()('ContentGraph.Node.Create')
      .enqueue(createNode => createNode({ ctx, input: fakeNode[nodeType](), nodeType }))
      .then(() => `NODE: ${info}`)
  }
  async function monkeyCreateEdge(ctx: MoodleNetExecutionContext) {
    ...
    return api<MoodleNetDomain>()('ContentGraph.Edge.Create')
      .enqueue(createEdge => createEdge({ ctx, input: fakeEdge[edgeType](), edgeType, from, to }))
      .then(() => `EDGE: ${info}`)
  }
})
