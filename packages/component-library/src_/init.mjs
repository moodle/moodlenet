...
// const tmpDir = resolve(tmpdir(), 'MN-component-library-modules')
...
const baseResolveAlias = {
    ...
    // '@moodlenet/component-library/src/webapp/ui': resolve(__dirname, '..', 'src', 'webapp', 'ui'),
    // '@moodlenet/component-library/lib/webapp/ui': resolve(__dirname, '..', 'src', 'webapp', 'ui'),
...
function writeGenerated() {
    ...
    //     `
    //   import lib from '${fixModuleLocForWebpackByOS(resolve(__dirname, '..', 'src', 'component-library-lib'))}'
    //   export default lib
    // `,
    //   ),
    // ])
}
export async function addWebappPluginItem(webappPluginItem) {
    // console.log({ webappPluginItem })
    pkgPlugins.push(webappPluginItem);
    // const scopedLibFilename = resolve(tmpDir, `component-library-lib__${dep.shell.extName}.ts`)
