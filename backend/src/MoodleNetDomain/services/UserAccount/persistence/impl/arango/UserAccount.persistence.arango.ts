...
export const arangoUserAccountPersistence: Promise<UserAccountPersistence> = DBReady.then(
  ({ db, Config, NewAccountRequest, Account }) => {
    const getAccountByUsername: UserAccountPersistence['getAccountByUsername'] = async ({
      username,
    }) => {
    ...
    const addChangeAccountEmailRequest: UserAccountPersistence['addChangeAccountEmailRequest'] = async ({
      flow,
    ...
    const confirmAccountEmailChangeRequest: UserAccountPersistence['confirmAccountEmailChangeRequest'] = async ({
      flow,
    }) => {
    ...
    const changeAccountEmailRequestExpired: UserAccountPersistence['changeAccountEmailRequestExpired'] = async ({
      flow,
    }) => {
    ...
    const isAccountOrOpenRequestWithEmailPresent = async ({
      email,
    }: {
      email: string
    }) => {
    ...
    const addNewAccountRequest: UserAccountPersistence['addNewAccountRequest'] = async ({
      req: { email },
      flow,
    }) => {
    ...
    const activateNewAccount: UserAccountPersistence['activateNewAccount'] = async ({
      requestFlowKey,
      password,
      username,
    }) => {
    ...
    const confirmNewAccountRequest: UserAccountPersistence['confirmNewAccountRequest'] = async ({
      flow,
    }) => {
      const confirmedStatus: NewAccountRequestDocumentStatus = 'Email Confirmed'
    ...
    const config: UserAccountPersistence['config'] = async () => {
      const cursor = await db.query(aql`
      FOR cfg IN Config
    ...
    const newAccountRequestExpired: UserAccountPersistence['newAccountRequestExpired'] = async ({
      flow,
    }) => {
    ...
    const changePassword: UserAccountPersistence['changePassword'] = async ({
      newPassword,
      username,
    }) => {
