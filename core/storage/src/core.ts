            ...
            coreCtx.write
              ...
              .then(() =>
                coreCtx.mod.env.query.modConfigs({ mod: 'storage' }).catch(e => {
                  log(
                    'alert',
                    'error query modConfigs, defaulting tempFileMaxRetentionSeconds to 10 minutes',
                    e,
                  )
                  return { configs: { tempFileMaxRetentionSeconds: 10 * 60 } }
                }),
              )
              .then(({ configs: { tempFileMaxRetentionSeconds } }) =>
                setTimeout(delStales, tempFileMaxRetentionSeconds * 1000),
              )
          }
        },
        primary(priCtx) {
