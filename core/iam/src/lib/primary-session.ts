import { contextModuleAccess, coreContext, ErrorXxx, iam, primaryContext } from '@moodle/domain'
...
export type sessionLibDepWithRole = sessionLibDep & { role: iam.user_role }
...
export async function validateAnyUserSession({ coreCtx, priCtx }: sessionLibDep) {
  ...
  const userSession: iam.userSession = {
    type: 'authenticated',
    user: validatedSignedTokenData.user,
  }
  return userSession
}
const guest_session: iam.userSession = {
  type: 'guest',
}
...
export async function assert_authorizeUserSessionWithRole(dep: sessionLibDepWithRole) {
  ...
  assert(
    authenticated_userSession,
    new ErrorXxx('Unauthorized', `assert_authorizeUserSessionWithRole ${dep.role}`),
  )
  return authenticated_userSession
}
