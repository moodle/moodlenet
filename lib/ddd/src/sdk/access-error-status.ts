...
export function statusByDescOrCodeXxx(
  ...
  return code_or_desc in status_code_by_desc_xxx
    ...
    : code_or_desc in status_desc_by_code_xxx
      ? code_or_desc
      : (() => {
          throw new Error(`Invalid xxx status code or desc: ${code_or_desc}`)
        })()
}
...
export function isCodeXxx(sc: number): sc is status_code_xxx {
...
export type status_desc_xxx = (typeof status_list_xxx)[number][0]
export type status_code_xxx = (typeof status_list_xxx)[number][1]

// export type status_code = status_code_2xx | status_code_xxx
export type statusXxx = status_desc_xxx | status_code_xxx
