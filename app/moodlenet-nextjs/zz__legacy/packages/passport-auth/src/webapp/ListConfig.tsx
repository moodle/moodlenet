...
export const ListConfig: FC = () => {
  ...
  const submitForm = (before: ConfigApiKey) => (configVals: ConfigApiKey) => {
    // Promise<[ErrorMsg | null, ConfigApiKey | undefined]> => {
    ...
    if (idxRepeat > -1)
      return [
        { field: provider, msg: 'provider ' + provider + ' is present ' } as ErrorMsg,
        undefined,
      ]
  ...
  return (
    <div>
...

      {
        ...
        list.map((config: ConfigApiKey) => (
          <FormConfig key={config.provider} />
        ))
      }
