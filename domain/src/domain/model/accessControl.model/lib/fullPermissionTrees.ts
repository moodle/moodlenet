export function getFullPermissionsConfigTree(schemas: Pick<moo.Models.statics.Schemas, 'org' | 'moodlenet' | 'userAccount'>) {
  ...
  const config: moo.permissions.config.tree = {
    ...
    authenticated: {
      _: {
        schemas: {
          uploadSize: schemas.userAccount.uploadSize,
        },
      },
