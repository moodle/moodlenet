...
import { db, SEARCH_VIEW_NAME, SearchAliasView } from './init/arangodb.mjs'
...
export async function registerEntity<EntityDataType extends SomeEntityDataType>(
  ...
  const { collection /* , newlyCreated */ } =
    await shell.call(ensureDocumentCollection)<EntityDocFullData<EntityDataType>>(
