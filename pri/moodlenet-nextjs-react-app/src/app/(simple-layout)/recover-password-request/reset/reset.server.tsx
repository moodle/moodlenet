...
export const resetMyPasswordAction = defaultSafeActionClient.schema(getResetMyPasswordSchema).action(async ({ parsedInput: resetPasswordForm }) => {
  const [ok, resp] = await client.proxy.userAccount.signedTokenAccess.resetPassword({
    resetPasswordForm,
  })
  if (!ok) {
